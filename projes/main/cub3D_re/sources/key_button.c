...
/*   Updated: 2023/01/11 23:59:40 by gsever           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	update_player(t_main *main)
{
	...
	if (main->key.up == true)
	{
		newPlayerX = main->ply.pos_x + (main->ply.walk_speed / BOX_SIZE) * cos(main->ply.rotation_angle * (M_PI / 180));
		newPlayerY = main->ply.pos_y - (main->ply.walk_speed / BOX_SIZE) * sin(main->ply.rotation_angle * (M_PI / 180));
	}
	...
	{
		newPlayerX = main->ply.pos_x - (main->ply.walk_speed / BOX_SIZE) * sin(main->ply.rotation_angle * (M_PI / 180));
		newPlayerY = main->ply.pos_y - (main->ply.walk_speed / BOX_SIZE) * cos(main->ply.rotation_angle * (M_PI / 180));
	}
