...
/*   Updated: 2022/10/27 01:45:04 by gsever           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	lexer_text_two(t_base *base, t_lexer *last,
	t_lexer **new, char *token)
{
	if (!token)
...
int	lexer_text(t_base *base, char *str, int *i)
{
	...
	if (len > 0)
	{
		...
		if (lexer_text_two(base, last, &new, token) == ERROR)
			return (ERROR);
		*i = *i + len;
